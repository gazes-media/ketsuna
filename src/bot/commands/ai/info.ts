...
export default async function Info(
  ...
  if (!userDatabase.horde_token)
    return i.edit({
      content: bt.__(
        {
          phrase: "User not registered, you must register with %s",
          locale: interaction.locale,
        },
  ...
  try {
      ...
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            [
              Capitalize(
                bt.__({ phrase: "Usage", locale: interaction.locale }),
              ),
              `- ${Capitalize(bt.__("Image generated"))}: ${
                user.records.fulfillment.image
              } (\`${user.records.contribution.megapixelsteps} MégaPixels\`)`,
              `- ${Capitalize(bt.__("Image requested"))}: ${
                user.records.request.image
              } (\`${user.records.usage.megapixelsteps} MégaPixels\`)`,
              `- ${Capitalize(bt.__("Text generated"))}: ${
                user.records.fulfillment.text
              } (\`${user.records.contribution.tokens} tokens\`)`,
              `- ${Capitalize(bt.__("Text requested"))}: ${
                user.records.request.text
              } (\`${user.records.usage.tokens} tokens\`)`,
              `- ${Capitalize(bt.__("Image interogated generated"))}: ${
                user.records.fulfillment.interrogation
              }`,
              `- ${Capitalize(bt.__("Image interogated requested"))}: ${
                user.records.request.interrogation
              }`,
              Capitalize("Kudos"),
              `- ${Capitalize(bt.__("Total"))}: ${user.kudos}`,
              `- ${Capitalize(bt.__("Earned"))}: ${
                user.kudos_details.accumulated
              }`,
              `- ${Capitalize(bt.__("Admin"))}: ${user.kudos_details.admin}`,
              `- ${Capitalize(bt.__("Given"))}: ${user.kudos_details.gifted}`,
              `- ${Capitalize(bt.__("Received"))}: ${
                user.kudos_details.received
              }`,
              `- ${Capitalize(bt.__("Recurring"))}: ${
                user.kudos_details.recurring
              }`,
    ...
    return i.edit({
      content: bt.__({
        phrase: "Token invalid you must login again",
        locale: interaction.locale,
      }),
    });
  }
