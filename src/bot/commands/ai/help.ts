...
export default async function Help(
  ...
  if (!AiCommand)
    return interaction.reply({
      content: bt.__({
        phrase: "An error occurred, command not found",
        locale: interaction.locale,
      }),
      flags: MessageFlags.Ephemeral,
    });
  let options = AiCommand.options;
  let optionsMapped = options
    .flatMap((option) => {
      if (option.type === ApplicationCommandOptionType.SubcommandGroup) {
        return option.options
          .map((subOption) => {
            ...
            try {
              let localizedDesc = Object.entries(
  ...
  let description = bt.__(
    {
      phrase:
        "Decriptions of commands\n%s\nFor the login command </%s login:%s>, you will first have to go to [AI Horde](https://stablehorde.net/register) enter a nickname, then copy the API key you will just have to paste it into the modal that will open",
      locale: interaction.locale,
    },
    optionsMapped,
    AiCommand.name,
